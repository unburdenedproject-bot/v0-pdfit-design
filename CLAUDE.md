...
## Test Suite
- 156 E2E tests (Playwright) + 6 unit tests (Vitest) = 162 total
- Run locally: `pnpm test:e2e` (needs dev server) or `pnpm test:unit`
- Config: `playwright.config.ts` — fullyParallel, 4 CI workers, hCaptcha test keys
...
- Coverage areas: smoke (32 health checks), tools (compress, merge, split, esign, url-to-pdf, table-extraction), auth (signup, login, password-reset, protected routes), payments (upgrade page, tier gating), SEO (structured data, hreflang, canonical), i18n (localized content, navigation, 404s), accessibility (WCAG AA axe audits on 6 pages), security (file upload), mobile (responsive), content (blog, legal, learn)
