...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const flag = await isToolEnabled("protect-pdf");
    if (!flag.enabled) {
    ...
    const usage = await checkUsageAndAuth("protect-pdf");
    if (!usage.allowed) {
    ...
    if (isJson) {
      ...
      if (!password || typeof password !== "string" || password.length < 4) {
        return errorResponse("Password required (min 4 chars)", 400);
      }
      ...
      if (body.async === true) {
        ...
        const result = await createJob({
          ...
          inputParams: { password, original_name: body.fileName || "input.pdf" },
        });
        if ("error" in result) {
          return errorResponse(result.error, 500);
        }
        await logUsage(usage.userId, "protect-pdf");
    ...
    // -----------------------------------------------------------
    // Common: run iLovePDF protect task
    // -----------------------------------------------------------
    ...
    if (usage) await logUsage(usage.userId, "protect-pdf");
