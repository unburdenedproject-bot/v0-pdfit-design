...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let originalName: string = "input.xlsx";
    ...
    if (isJson) {
      ...
      if (body.async === true) {
        ...
        const result = await createJob({
          ...
          inputParams: { original_name: body.originalName || "input.xlsx" },
        });
        if ("error" in result) {
          return errorResponse(result.error, 500);
        }
        const { logUsage } = await import("@/lib/usage-check");
        await logUsage(user.id, "excel-to-pdf");
      ...
      if (!ALLOWED_EXTENSIONS.has(ext)) {
        ...
        return errorResponse(
          `Only Excel files are accepted. Got ".${ext}".`,
          400
        );
      }
    } else {
      ...
      originalName = file.name || "input.xlsx";
      const ext: string = getExtension(originalName);
