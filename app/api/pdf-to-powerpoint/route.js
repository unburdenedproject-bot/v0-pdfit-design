...
export async function POST(request) {
  ...
  try {
    ...
    const usage = await checkUsageAndAuth("pdf-to-powerpoint");
    if (!usage.allowed) {
    ...
    await task.process({ pdfoffice_mode: "powerpoint" });
    ...
    if (usage) await logUsage(usage.userId, "pdf-to-powerpoint");
