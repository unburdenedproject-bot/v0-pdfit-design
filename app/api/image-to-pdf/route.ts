...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const blobUrls: string[] = body.blobUrls || (body.blobUrl ? [body.blobUrl] : [])
    blobUrl = blobUrls[0] // for cleanup in catch block
    ...
    try {
      const pathname = new URL(blobUrls[0]).pathname
    ...
    const suffix = blobUrls.length > 1 ? "-combined" : ""
    ...
    const response = new NextResponse(Buffer.from(pdfBytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${originalName}${suffix}.pdf"`,
    ...
    if (blobUrl) {
      try {
        ...
        const urls = body.blobUrls || (body.blobUrl ? [body.blobUrl] : [])
        for (const u of urls) { await del(u).catch(() => {}) }
      } catch {
