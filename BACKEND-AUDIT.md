...
**Post-Launch Architecture Shift (April 14, 2026):** Five AI-over-PDF routes — `question-generator`, `ats-optimizer`, `smart-extraction`, `chat-with-pdf`, `translate-pdf` — migrated off the iLoveAPI → pdf-parse → pdfjs-dist → Document AI text-extraction chain. All five now upload PDFs directly to OpenAI's Files API (`POST /v1/files` with `purpose=user_data`) and reference the returned `file_id` in chat-completions messages via `{type:"file", file:{file_id}}`. The previous chain failed consistently on Vercel serverless for PDFs with custom fonts, scanned pages, or unusual structure (e.g. real user resumes). New model: single external dependency per AI call, no intermediate text extraction, native OCR handled by GPT-4o-mini. Blank-PDF guard (`lib/blank-pdf-check.js`) is now bypassed for these routes — the model itself detects blank/unreadable input via a sentinel-response pattern. `lib/pdf-text-extract.ts` remains in place for legacy routes and has Google Document AI as the last-resort fallback. See COMPLETED.md and LEARNINGS.md (2026-04-14 entries) for full detail.
