...
/**
 * PowerPoint to PDF processor — extracted from app/api/powerpoint-to-pdf/route.ts
 * Uses iLoveAPI "officepdf" task.
...
export async function powerpointToPdfProcessor(
  ...
  const originalName = (params.original_name as string) || "input.pptx"
  ...
  logger.info("powerpoint_to_pdf_complete", { originalName, outputUrl: blob.url })
