...
/**
 * Protect PDF processor — extracted from app/api/protect-pdf/route.ts
 * Used by the job queue system for async processing.
 */
export async function protectPdfProcessor(
  tmpPath: string,
  ...
  if (!password || typeof password !== "string" || password.length < 4) {
    throw new Error("Password required (min 4 chars)")
  }
  ...
  try {
    ...
    if (blank) {
      throw new Error("This file appears to be empty and cannot be protected")
    }
  } catch (err) {
  ...
  logger.info("protect_pdf_complete", {
    originalName,
    outputUrl: blob.url,
  })
