...
/**
 * Excel to PDF processor — extracted from app/api/excel-to-pdf/route.ts
...
export async function excelToPdfProcessor(
  ...
  const originalName = (params.original_name as string) || "input.xlsx"
  ...
  logger.info("excel_to_pdf_complete", { originalName, outputUrl: blob.url })
