...
/**
 * PDF to Word processor — extracted from app/api/pdf-to-word/route.ts
 * Uses CloudConvert REST API (pdf → docx).
...
export async function pdfToWordProcessor(
  ...
  for (let i = 0; i < 60; i++) {
    ...
    // Report progress during polling (30-70%)
  ...
  logger.info("pdf_to_word_complete", { originalName, outputUrl: blob.url })
