...
/**
 * Compress PDF processor — extracted from app/api/compress-pdf/route.ts
...
export async function compressPdfProcessor(
  ...
  try {
    ...
    if (blank) {
      throw new Error("This file appears to be empty and cannot be compressed")
    }
  } catch (err) {
    if ((err as Error).message.includes("empty")) throw err
    // PDF parse error — treat as potentially valid, let iLoveAPI handle it
  }
  ...
  logger.info("compress_pdf_complete", {
    originalName,
    compressionLevel,
    outputUrl: blob.url,
  })
