...
/**
 * PDF to PNG processor — extracted from app/api/pdf-to-png/route.ts
 * Used by the job queue system for async processing.
 */
export async function pdfToPngProcessor(
  tmpPath: string,
  ...
  // Process with iLoveAPI — pdfjpg with "pages" mode + png output
  ...
  if (isZip) {
    ...
    for (const entryName of entries) {
      ...
      if (!entry.dir) {
        const newName = `${originalBase}-page-${String(pageNum).padStart(4, "0")}.png`
  ...
  const outputFilename = `${originalBase}-png-images.zip`
  ...
  logger.info("pdf_to_png_complete", {
    originalName,
    outputUrl: blob.url,
  })
